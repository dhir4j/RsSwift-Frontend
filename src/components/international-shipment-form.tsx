...
const formSchema = z.object({
  ...
  senderPostcode: z.string().min(3, "Postcode is required"),
  ...
  receiverCountry: z.string().min(2, "Country is required"),
  receiverPostcode: z.string().min(3, "Postcode is required"),
  receiverPhone: z.string().min(5, "Phone is required"),
  // Product
...
export default function InternationalShipmentForm() {
    ...
    const form = useForm<z.infer<typeof formSchema>>({
     ...
     const handleCalculatePrice = async () => {
        const { receiverCountry, weight } = form.getValues();
        if (!receiverCountry || !weight) {
            toast({
                variant: "destructive",
                title: "Missing Information",
                description: "Please fill in Receiver's Country and Weight to calculate the price.",
            });
            return;
        }
        ...
        setPriceData(null);
        try {
            const response = await fetch('https://www.server.shedloadoverseas.com/api/international/price', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ country: receiverCountry, weight }),
            });
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        const payload = {
            ...
            sender_address_state: "", // Not applicable for sender in India for this form
            sender_address_pincode: values.senderPostcode,
            sender_address_country: values.senderCountry,
            ...
            receiver_address_state: "", // Not always applicable for international
            receiver_address_pincode: values.receiverPostcode,
            receiver_address_country: values.receiverCountry,
            ...
            service_type: "Express", // Hardcoded as per API spec
